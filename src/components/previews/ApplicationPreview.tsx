...
import { platformsForLinks } from "@/components/accordions/LinksPlatformsForm";
import CleanApplicationPreview from "./placeholders/CleanApplicationPreview";

interface ApplicationPreviewProps {
  appInfo?: ApplicationPageData["appInfo"];
  platformLinks?: ApplicationPageData["platformLinks"];
  design?: ApplicationPageData["design"];
  fonts?: ApplicationPageData["fonts"];
}
...
export default function ApplicationPreview(props: ApplicationPreviewProps) {
  const {
    ...
    design = {
      primaryColor: "#5502f3",
      secondaryColor: "#5c2bb7",
    },
    fonts,
  } = props;

  const showPreview =
    !appInfo?.appName && !appInfo?.appDescription && platformLinks.length === 0;

  if (showPreview) {
    return (
      <CleanApplicationPreview
        bgPrimaryColor={design.primaryColor}
        bgSecondaryColor={design.secondaryColor}
      />
    );
  }
  ...
  const getPlatformText = (platformId: string) => {
    switch (platformId) {
      case "apple":
        return {
  ...
  return (
    <div className="relative max-w-xs mx-auto h-full">
      <WaveBG
        ...
        topColor={design.primaryColor}
        bottomColor={design.secondaryColor}
      />
      {/* Phone frame */}
            ...
            <div className="space-y-2">
              <div className="space-y-1 pb-14" style={{ color: "white" }}>
                ...
                <h1
                  ...
                  style={{
                    fontSize: fonts?.titleFontSize || "16px",
                ...
                <h2
                  className="text-sm block font-medium break-words"
                  style={{
                    fontSize: fonts?.descriptionFontSize || "12px",
              ...
              <div className="bg-white border border-gray-300 space-y-2 px-3 py-5 rounded-md relative text-black">
                ...
                {/* App Description */}
                <p
                  className="text-sm pt-8 pb-2 font-medium break-words"
                  style={{
                    fontSize: fonts?.descriptionFontSize || "12px",
                    fontFamily: fonts?.descriptionFont,
                  }}
                ...
                {platformLinks.map((link) => {
                  ...
                  const platformText = getPlatformText(link.platform);
                  return (
                    <a
                      key={link.platform}
              ...
              {appInfo?.website && (
                ...
                >
                  ...
                  >
                    ...
                    <span className="max-w-[123px] text-xs text-blue-500 group-hover:underline truncate">
                      {appInfo.website}
                    </span>
